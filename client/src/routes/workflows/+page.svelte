...
<section class="space-y-8 px-4 pb-12 pt-6 md:px-12 md:pt-10">
  <header class="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
    <div class="space-y-2">
      <span class="pill">workflows</span>
      <p class="text-sm text-muted max-w-2xl">
        Собирайте автоматизации из триггеров, фильтров и действий. Управляйте версиями, создавайте черновики и переиспользуемые блоки для команд.
      </p>
    </div>
    <div class="flex flex-wrap gap-3">
      <a href="/templates" class="btn-primary">Перейти к шаблонам</a>
      <a href="/workflows/new" class="btn-primary bg-surfaceMuted text-text shadow-none hover:shadow-sm">Новый workflow</a>
    </div>
  </header>
  ...
  <div class="grid gap-8 lg:grid-cols-2">
    <div class="space-y-4">
      <div class="flex items-center justify-between">
        <div>
          ...
          <p class="text-xs uppercase tracking-wide text-muted">Готовы к использованию • {activeWorkflows.length}</p>
        </div>
      </div>
          ...
          <article class="glass-card h-full">
            <div class="flex items-start justify-between gap-3">
              ...
              <div class="flex items-center gap-2">
                ...
                <span class="inline-flex rounded-full bg-positive/10 px-3 py-1 text-xs font-medium text-positive">active</span>
              </div>
            </div>
            ...
            <div class="mt-4 flex items-center justify-between">
              <button type="button" class="inline-flex text-sm font-semibold text-accent hover:underline">Открыть конструктор</button>
            </div>
          </article>
        {/each}
      </div>
    ...
    <div class="space-y-4">
      <div class="flex items-center justify-between">
        ...
        <button type="button" class="text-sm font-semibold text-accent hover:underline">Все черновики</button>
      </div>
      <div class="grid gap-4 sm:grid-cols-2">
        {#each draftBlueprints as draft}
          <article class="glass-card h-full">
            ...
            <button type="button" class="mt-4 inline-flex text-sm font-semibold text-accent hover:underline">Открыть черновик</button>
          </article>
        {/each}
        <article class="glass-card flex h-full items-center justify-center border-dashed border-border text-muted">
