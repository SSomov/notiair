  ...
  function selectTrigger(option: string) {
    ...
    nodes = nodes.map((node) =>
      node.variant === 'trigger'
        ? { ...node, description: option === 'Добавить триггер' ? 'Выберите событие запуска' : option }
        : node
    );
  }
...
<section class="space-y-6 px-4 pb-12 pt-6 md:px-12 md:pt-10">
  <header class="space-y-2">
    <span class="pill">новый workflow</span>
    <p class="max-w-2xl text-sm text-muted">
      Добавьте триггеры, действия и каналы доставки. Каждую ноду можно связать линиями и протестировать перед публикацией.
    </p>
  </header>
  ...
  <div class="workspace">
    <div class="workspace-toolbar">
      <div class="relative">
        ...
        {#if triggerMenuOpen}
          <ul class="absolute z-10 mt-2 w-48 overflow-hidden rounded-xl border border-border bg-surface shadow-lg">
      ...
      <div class="flex items-center gap-3">
        <button type="button" class="btn-primary bg-surfaceMuted text-text shadow-none hover:shadow-sm">
