...
<header
	class="sticky top-0 z-40 w-full border-b border-border bg-surface/95 shadow-sm backdrop-blur"
>
	<div class="mx-auto flex w-full max-w-7xl items-center justify-between px-4 py-4 md:px-10">
		<a
			href={currentLocale === 'ru' ? '/ru/' : '/'}
			class="flex items-center gap-4 transition hover:opacity-80"
		>
			<span
				class="inline-flex h-11 w-11 items-center justify-center rounded-xl bg-accent/10 text-2xl text-accent"
			>
				<svg
				...
				>
					<path
						d="M13.5 2a1 1 0 0 1 .894.553l6 12A1 1 0 0 1 19.5 16h-5.382l.618 5.543a1 1 0 0 1-1.872.654l-9-14A1 1 0 0 1 4.75 6H10.5l-1.118-3.276A1 1 0 0 1 10.341 2H13.5Z"
					/>
				</svg>
			</span>
			<div>
				<p class="text-xs font-semibold uppercase tracking-[0.45em] text-accent">
					{$t('header.title')}
				</p>
		...
		<div class="flex items-center gap-3">
			<a
				href={currentLocale === 'ru' ? '/ru/connectors' : '/connectors'}
				class="btn-primary hidden md:inline-flex">{$t('common.connectors')}</a
			>
			<a
				href={currentLocale === 'ru' ? '/ru/channels' : '/channels'}
				class="btn-primary hidden md:inline-flex">{$t('common.channels')}</a
			>
			<a
				href={currentLocale === 'ru' ? '/ru/workflows' : '/workflows'}
				class="btn-primary hidden md:inline-flex">{$t('common.workflows')}</a
			>
			...
			<div class="relative" bind:this={settingsWrap}>
				...
				{#if settingsOpen}
					<div class="settings-dropdown" role="menu">
						...
						<a href={apiKeysHref} class="settings-dropdown-item" role="menuitem"
							>{$t('header.settingsApiKeys')}</a
						>
					</div>
				{/if}
			</div>
